...
class EasyservoPlugin(octoprint.plugin.SettingsPlugin,
    ...
    def on_settings_save(self, data):
        ...
        if oldGPIOX != newGPIOX:
            ...
            self.pi.set_servo_pulsewidth(newGPIOX, self.angle_to_width(
                xAutoAngle))  # Angle / width ??????
        if oldGPIOY != newGPIOY:
            ...
            self.pi.set_servo_pulsewidth(
                newGPIOY, self.angle_to_width(yAutoAngle))
    ...
    def on_after_startup(self):
        ...
        if libraryUsed == "rpigpio":
            ...
            GPIOY = self._settings.get_int(["GPIOY"])
            self.pi.set_servo_pulsewidth(
                GPIOX, self.angle_to_width(xAutoAngle))
            self.pi.set_servo_pulsewidth(
                GPIOY, self.angle_to_width(yAutoAngle))
    ...
    def on_shutdown(self):
        if not self.pi.connected:
            self._logger.info(
                "There was an error on shutdown pigpio not connected")
            return
        GPIOX = self._settings.get_int(["GPIOX"])
        GPIOY = self._settings.get_int(["GPIOY"])
            ...
            if width_current > self.angle_to_width(maxAngle):
                self._logger.info(
                    "GPIO {} reached his boundaries with a {} pulse width = {}°".format(pin, width_current,
                                                                                        angle_to_reach))
                self.pi.set_servo_pulsewidth(
                    int(pin), self.angle_to_width(maxAngle) - 10)
                break
                ...
                self._logger.info(
                    "GPIO {} reached his boundaries with a {} pulse width = {}°".format(pin, width_current,
                                                                                        angle_to_reach))
                self.pi.set_servo_pulsewidth(
                    int(pin), self.angle_to_width(minAngle) + 10)
                break
    ...
    def move_servo_to_ang_pimoroni(self, axis, angle_to_reach):
        if axis == "PAN":
            ...
            for x in range(actual_angle, angle_to_reach, incrementSign):
                ...
                if angle_current > maxAngle:
                    self._logger.info(
                        "PAN reached his boundaries with a {} pulse width".format(angle_current))
        ...
        if axis == "TILT":
            ...
            for x in range(actual_angle, angle_to_reach, incrementSign):
                ...
                if angle_current > maxAngle:
                    self._logger.info(
                        "TILT reached his boundaries with a {} pulse width".format(angle_current))
    ...
    def move_servo_by(self, pin, angle_difference):  # Relative positioning
        ...
        if inverted:
            actual_angle = self.angle_to_inverted(
                self.width_to_angle(actual_width))
            angle_to_reach = actual_angle + angle_difference
            width_to_reach = self.width_to_inverted(
                self.angle_to_width(angle_to_reach))
        else:
            actual_angle = self.width_to_angle(actual_width)
            angle_to_reach = actual_angle + angle_difference
            width_to_reach = self.angle_to_width(angle_to_reach)
                ...
                if pigpioUsed:
                    ...
                    if int(GPIO) == self._settings.get_int(["GPIOX"]) or int(GPIO) == self._settings.get_int(["GPIOY"]):
                        thread = threading.Thread(
                            target=self.move_servo_by, args=(int(GPIO), int(ang)))
                        thread.daemon = True
                        thread.start()
                    else:
                    ...
                    if axis == "PAN" or axis == "TILT":
                        thread = threading.Thread(
                            target=self.move_servo_by_pimoroni, args=(str(axis), int(ang)))
                        thread.daemon = True
                        thread.start()
                    else:
                        self._logger.info(
                            "please use PAN or TILT instead of '" + str(axis)) + "'"
        ...
        if line.startswith('EASYSERVO_ABS'):
            if len(line.split()) == 3:
                if pigpioUsed:
                    ...
                    if int(GPIO) == self._settings.get_int(["GPIOX"]) or int(GPIO) == self._settings.get_int(["GPIOY"]):
                        thread = threading.Thread(
                            target=self.move_servo_to_ang, args=(int(GPIO), int(ang)))
                        thread.daemon = True
                        thread.start()
                    else:
                        self._logger.info("unknown GPIO %d" % int(GPIO))
                    ...
                    if str(axis) == "PAN" or str(axis) == "TILT":
                        thread = threading.Thread(
                            target=self.move_servo_to_ang_pimoroni, args=(str(axis), int(ang)))
                        thread.daemon = True
                        thread.start()
                    else:
                        self._logger.info(
                ...
                self._logger.info(
                    "please use EASYSERVO_ABS PIN/AXIS ANGLE instead of '{}'".format(str(line)))
                ...
                if pigpioUsed:
                    ...
                    if (int(GPIO1) == self._settings.get_int(["GPIOX"]) and int(GPIO2) == self._settings.get_int(
                        ...
                        thread_x = threading.Thread(
                            target=self.move_servo_to_ang, args=(int(GPIO1), xAutoAngle))
                        thread_x.daemon = True
                        thread_x.start()
                        thread_y = threading.Thread(
                            target=self.move_servo_to_ang, args=(int(GPIO2), yAutoAngle))
                        thread_y.daemon = True
                        thread_y.start()
                    else:
                        self._logger.info(
                            "unknown GPIO1 {} or GPIO2 {}".format(int(GPIO1), int(GPIO2)))
                else:
                    ...
                    if (str(axis1) == "PAN" and str(axis2) == "TILT") or (str(axis1) == "TILT" and str(axis2) == "PAN"):
                        if not self._settings.get_boolean(["axisInvert"]):
                            ...
                            yAxis = "PAN"
                        thread_x = threading.Thread(
                            target=self.move_servo_to_ang_pimoroni, args=(xAxis, xAutoAngle))
                        thread_x.daemon = True
                        thread_x.start()
                        thread_y = threading.Thread(
                            target=self.move_servo_to_ang_pimoroni, args=(yAxis, yAutoAngle))
                        thread_y.daemon = True
                        thread_y.start()
                    else:
                        self._logger.info(
                            "unknown AXIS1 {} or AXIS2 {}".format(str(axis1), str(axis2)))
            elif len(line.split()) == 2:
                if pigpioUsed:
                    command, GPIO = line.split()
                    if int(GPIO) == self._settings.get_int(["GPIOX"]):
                        thread = threading.Thread(
                            target=self.move_servo_to_ang, args=(int(GPIO), xAutoAngle))
                        thread.daemon = True
                        thread.start()
                    elif int(GPIO) == self._settings.get_int(["GPIOY"]):
                        thread = threading.Thread(
                            target=self.move_servo_to_ang, args=(int(GPIO), yAutoAngle))
                        thread.daemon = True
                        thread.start()
                    else:
                        self._logger.info("unknown GPIO %d" % int(GPIO))
                    ...
                    if str(axis) == "PAN":
                        if not self._settings.get_boolean(["axisInvert"]):
                            ...
                            xAxis = "TILT"
                        thread = threading.Thread(
                            target=self.move_servo_to_ang_pimoroni, args=(str(xAxis), xAutoAngle))
                        thread.daemon = True
                        thread.start()
                    if str(axis) == "TILT":
                        if not self._settings.get_boolean(["axisInvert"]):
                            ...
                            yAxis = "PAN"
                        thread = threading.Thread(
                            target=self.move_servo_to_ang_pimoroni, args=(str(yAxis), yAutoAngle))
                        thread.daemon = True
                        thread.start()
                    else:
        ...
        if ((cmd.startswith("G0") or cmd.startswith("G1")) and "Z" in cmd) and self._settings.get_boolean(
            ...
            if pigpioUsed:
                ...
                if not self._settings.get_boolean(["axisInvert"]):
                    ...
                    yAxis = "PAN"
                thread = threading.Thread(
                    target=self.move_servo_to_ang_pimoroni, args=(str(yAxis), int(self.currentZ)))
                thread.daemon = True
                thread.start()
